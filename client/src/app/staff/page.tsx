...
const Staff = () => {
      ...
      try {
        ...
        if (user?.MaNhanVien) {
          ...
          if (activePhien) {
            ...
            // Không có phiên làm việc đang mở -> reset state
  ...
  // Reload phiên làm việc khi focus lại trang hoặc visibility change (sau khi checkout)
  useEffect(() => {
    const reloadShiftStatus = async () => {
      ...
      try {
        ...
        console.error('Error reloading shift status:', err)
      }
    }
    ...
    // Reload ngay khi component mount hoặc route change
    reloadShiftStatus()

    window.addEventListener('focus', reloadShiftStatus)
    document.addEventListener('visibilitychange', handleVisibilityChange)

    return () => {
      window.removeEventListener('focus', reloadShiftStatus)
  ...
  const openCustomizeModal = (product: Product) => {
    ...
    const defaultSize = sizes.find(s => s.TenTuyChon === 'M')?.MaTuyChon || sizes[0]?.MaTuyChon || ''
  ...
  // Đóng phiên làm việc hiện tại (checkout)
  const handleCheckOutShift = async () => {
    if (!currentPhienLamViec) {
      toast.error('Không có phiên làm việc đang mở!')
      return
    }
    setIsProcessing(true)
    try {
      await phienLamViecApi.closeShift(currentPhienLamViec)
      toast.success('Đã đóng phiên làm việc!')
      setCurrentPhienLamViec(null)
      setCurrentShiftInfo(null)
    } catch (err) {
      toast.error('Lỗi khi đóng phiên làm việc!')
    } finally {
      setIsProcessing(false)
    }
  }
    ...
    <ProtectedRoute>
      <div className={Style.staffContainer}>
        ...
        <div className={Style.quickActions}>
          ...
          {/* Nút checkout đóng phiên làm việc */}
          {currentPhienLamViec && (
            <button
              className={Style.quickActionCard}
              style={{ background: '#fee2e2', color: '#991b1b', border: '1px solid #fecaca', marginLeft: 8, minWidth: 180 }}
              onClick={handleCheckOutShift}
              disabled={isProcessing}
              title="Đóng phiên làm việc (Checkout)"
            >
              <FaSignOutAlt style={{ marginRight: 8 }} /> Đóng phiên làm việc
            </button>
          )}
        </div>
                    ...
                    <div className={Style.orderTypeButtons}>
                      ...
                      {sizes.map((size) => (
                        <button
                          key={size.MaTuyChon}
                          type="button"
                          className={`${Style.orderTypeBtn} ${customizeOptions.size === size.MaTuyChon ? Style.active : ''}`}
                          onClick={() => setCustomizeOptions({ ...customizeOptions, size: size.MaTuyChon })}
                        >
                          {size.TenTuyChon} {size.GiaCongThem > 0 && `(+${formatPrice(size.GiaCongThem)})`}
                        </button>
                      ))}
                    </div>
                  </div>
