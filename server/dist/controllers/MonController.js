...
exports.MonController = void 0;
const data_source_1 = require("../database/data-source");
const Mon_1 = require("../entities/Mon");
class MonController {
    constructor() {
        this.repository = data_source_1.AppDataSource.getRepository(Mon_1.Mon);
    }
    ...
    async getOne(req, res) {
        const { id } = req.params;
        const item = await this.repository.findOne({ where: { MaMon: id } });
        if (!item)
    ...
    async update(req, res) {
        const { id } = req.params;
        const existed = await this.repository.findOne({ where: { MaMon: id } });
        if (!existed)
            return res.status(404).json({ message: "Không tìm thấy" });
...
exports.MonController = MonController;
