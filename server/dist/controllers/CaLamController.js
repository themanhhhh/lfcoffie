...
exports.CaLamController = void 0;
const data_source_1 = require("../database/data-source");
const CaLam_1 = require("../entities/CaLam");
class CaLamController {
    constructor() {
        this.repository = data_source_1.AppDataSource.getRepository(CaLam_1.CaLam);
    }
    ...
    async getOne(req, res) {
        try {
            const { id } = req.params;
            const item = await this.repository.findOne({ where: { MaCaLam: id } });
            if (!item)
    ...
    async update(req, res) {
        try {
            const { id } = req.params;
            const existed = await this.repository.findOne({ where: { MaCaLam: id } });
            if (!existed)
                return res.status(404).json({ message: "Không tìm thấy" });
...
exports.CaLamController = CaLamController;
