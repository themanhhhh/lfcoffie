...
exports.GiamHoaDonController = void 0;
const data_source_1 = require("../database/data-source");
const GiamHoaDon_1 = require("../entities/GiamHoaDon");
const typeorm_1 = require("typeorm");
class GiamHoaDonController {
    constructor() {
        this.repository = data_source_1.AppDataSource.getRepository(GiamHoaDon_1.GiamHoaDon);
    }
    ...
    async update(req, res) {
        try {
            const { id } = req.params;
            const existed = await this.repository.findOne({ where: { MaGHD: id } });
            if (!existed)
                return res.status(404).json({ message: "Không tìm thấy" });
...
exports.GiamHoaDonController = GiamHoaDonController;
