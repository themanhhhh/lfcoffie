...
exports.NghiepVuController = void 0;
const data_source_1 = require("../database/data-source");
const NghiepVu_1 = require("../entities/NghiepVu");
class NghiepVuController {
    constructor() {
        this.repository = data_source_1.AppDataSource.getRepository(NghiepVu_1.NghiepVu);
    }
    ...
    async getOne(req, res) {
        try {
            const { id } = req.params;
            const item = await this.repository.findOne({ where: { MaNghiepVu: id } });
            if (!item)
    ...
    async update(req, res) {
        try {
            const { id } = req.params;
            const existed = await this.repository.findOne({ where: { MaNghiepVu: id } });
            if (!existed)
                return res.status(404).json({ message: "Không tìm thấy" });
...
exports.NghiepVuController = NghiepVuController;
