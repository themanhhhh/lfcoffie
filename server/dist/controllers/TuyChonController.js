...
exports.TuyChonController = void 0;
const data_source_1 = require("../database/data-source");
const TuyChon_1 = require("../entities/TuyChon");
class TuyChonController {
    constructor() {
        this.repository = data_source_1.AppDataSource.getRepository(TuyChon_1.TuyChon);
    }
    ...
    async getOne(req, res) {
        try {
            const { id } = req.params;
            const item = await this.repository.findOne({ where: { MaTuyChon: id } });
            if (!item)
    ...
    async update(req, res) {
        try {
            const { id } = req.params;
            const existed = await this.repository.findOne({ where: { MaTuyChon: id } });
            if (!existed)
                return res.status(404).json({ message: "Không tìm thấy" });
...
exports.TuyChonController = TuyChonController;
