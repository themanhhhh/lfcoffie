...
export class TuyChonController {
  ...
  async getOne(req: Request, res: Response) {
    try {
      const { id } = req.params;
      const item = await this.repository.findOne({ where: { MaTuyChon: id, isDelete: false } as any });
  ...
  async update(req: Request, res: Response) {
    try {
      const { id } = req.params;
      const existed = await this.repository.findOne({ where: { MaTuyChon: id, isDelete: false } as any });
