...
export class GiamMonController {
  private repository = AppDataSource.getRepository(GiamMon);
  ...
  async getOne(req: Request, res: Response) {
    try {
      ...
      const item = await this.repository.findOne({ 
        where: { MaGM: id, isDelete: false } as any,
        relations: ['ctkm', 'mon']
      });
  ...
  async create(req: Request, res: Response) {
    try {
      const obj = this.repository.create(req.body as GiamMon);
  ...
  async update(req: Request, res: Response) {
    try {
      const { id } = req.params;
      const existed = await this.repository.findOne({ where: { MaGM: id, isDelete: false } as any });
  ...
  async getActiveRulesForMon(req: Request, res: Response) {
    try {
      ...
      const activeRules = rules.filter(rule => {
        if (rule.Thu && rule.Thu !== currentDay) return false;
        if (rule.GioBatDau && rule.GioKetThuc) {
          if (currentTime < rule.GioBatDau || currentTime > rule.GioKetThuc) return false;
        }
        return true;
      });
