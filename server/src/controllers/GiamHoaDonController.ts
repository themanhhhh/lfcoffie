...
export class GiamHoaDonController {
  private repository = AppDataSource.getRepository(GiamHoaDon);
  ...
  async create(req: Request, res: Response) {
    try {
      const obj = this.repository.create(req.body as GiamHoaDon);
  ...
  // Lấy các quy tắc giảm giá đang áp dụng
  async getActiveRules(req: Request, res: Response) {
    try {
      const now = new Date();
