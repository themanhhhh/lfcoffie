...
export class NguyenLieuController {
  private repository = AppDataSource.getRepository(NguyenLieu);
  ...
  async getOne(req: Request, res: Response) {
    const { id } = req.params;
    const item = await this.repository.findOne({ where: { maNL: id } } as any);
  ...
  async create(req: Request, res: Response) {
    try {
      const obj = this.repository.create(req.body as NguyenLieu);
  ...
  async update(req: Request, res: Response) {
    const { id } = req.params;
    const existed = await this.repository.findOne({ where: { maNL: id } } as any);
