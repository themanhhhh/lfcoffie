...
export class ChiTietPhieuNhapController {
  private repository = AppDataSource.getRepository(ChiTietPhieuNhap);
  ...
  async getOne(req: Request, res: Response) {
    const { id } = req.params;
    const item = await this.repository.findOne({ where: { maCTPN: id } } as any);
  ...
  async create(req: Request, res: Response) {
    try {
      const obj = this.repository.create(req.body as ChiTietPhieuNhap);
  ...
  async update(req: Request, res: Response) {
    const { id } = req.params;
    const existed = await this.repository.findOne({ where: { maCTPN: id } } as any);
