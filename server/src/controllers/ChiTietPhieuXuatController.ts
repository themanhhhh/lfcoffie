...
export class ChiTietPhieuXuatController {
  private repository = AppDataSource.getRepository(ChiTietPhieuXuat);
  ...
  async getOne(req: Request, res: Response) {
    const { id } = req.params;
    const item = await this.repository.findOne({ where: { maCTPX: id } } as any);
  ...
  async create(req: Request, res: Response) {
    try {
      const obj = this.repository.create(req.body as ChiTietPhieuXuat);
  ...
  async update(req: Request, res: Response) {
    const { id } = req.params;
    const existed = await this.repository.findOne({ where: { maCTPX: id } } as any);
