...
export class TheBanController {
  private repository = AppDataSource.getRepository(TheBan);
  ...
  async getOne(req: Request, res: Response) {
    const { id } = req.params;
    const item = await this.repository.findOne({ where: { MaTheBan: id, isDelete: false } } as any);
  ...
  async create(req: Request, res: Response) {
    try {
      const obj = this.repository.create(req.body as TheBan);
  ...
  async update(req: Request, res: Response) {
    const { id } = req.params;
    const existed = await this.repository.findOne({ where: { MaTheBan: id, isDelete: false } } as any);
