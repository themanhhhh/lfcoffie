...
export class NhomThucDonController {
  private repository = AppDataSource.getRepository(NhomThucDon);
  ...
  async getOne(req: Request, res: Response) {
    const { id } = req.params;
    const item = await this.repository.findOne({ where: { maNhomThucDon: id } } as any);
  ...
  async create(req: Request, res: Response) {
    try {
      const obj = this.repository.create(req.body as NhomThucDon);
  ...
  async update(req: Request, res: Response) {
    const { id } = req.params;
    const existed = await this.repository.findOne({ where: { maNhomThucDon: id } } as any);
