...
export class ChiTietPhieuThuController {
  private repository = AppDataSource.getRepository(ChiTietPhieuThu);
  ...
  async getOne(req: Request, res: Response) {
    const { id } = req.params;
    const item = await this.repository.findOne({ where: { maCTPT: id } } as any);
  ...
  async create(req: Request, res: Response) {
    try {
      const obj = this.repository.create(req.body as ChiTietPhieuThu);
  ...
  async update(req: Request, res: Response) {
    const { id } = req.params;
    const existed = await this.repository.findOne({ where: { maCTPT: id } } as any);
